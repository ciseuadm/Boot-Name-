...
// ─── Message texts ─────────────────────────────────────────────────────────
const HELP = `🐤 <b>Add Button Bot</b>

Добавляю кнопки к постам в твоём канале — без рекламы и пометок.

<b>Команды:</b>
/add — добавить кнопки к посту
/remove — убрать все кнопки с поста
/help — показать это сообщение

<b>Как начать:</b>
1. Добавь меня в администраторы своего канала
2. Дай мне право <i>Редактировать сообщения</i>
3. Пиши /add и следуй инструкции`;
...
async function handleUpdate(update) {
    ...
    if (text === '/add') {
        ...
        await sendMessage(chatId, '🔗 Отправь ссылку на пост в твоём канале.\n\n' +
    ...
    if (text === '/remove') {
        ...
        await sendMessage(chatId, '🔗 Отправь ссылку на пост, с которого нужно убрать кнопки.\n\n' +
            '❌ /cancel — отмена');
        return;
    }
    if (text === '/cancel') {
    ...
    if (state.step === 'waiting_link_add') {
        ...
        if (!parsed) {
            await sendMessage(chatId, '❌ Не распознал ссылку.\n\n' +
                'Ожидаю формат:\n' +
        ...
        await sendMessage(chatId, `✅ Пост найден!\n\n${BUTTON_FORMAT_HELP}\n\nОтправь кнопки 👇\n\n❌ /cancel — отмена`);
        return;
    }
    ...
    if (state.step === 'waiting_buttons') {
        const rows = (0, parser_1.parseButtons)(text);
        if (!rows) {
            await sendMessage(chatId, `❌ Не смог разобрать кнопки. Проверь формат.\n\n${BUTTON_FORMAT_HELP}\n\n❌ /cancel — отмена`);
            return;
        }
        const { chatId: postChatId, messageId } = state;
        states.set(userId, { step: 'idle' });
        try {
            const markup = { inline_keyboard: rows.map(row => row.map((b) => ({ text: b.text, url: b.url }))) };
            ...
            await sendMessage(chatId, `✅ Готово! Добавил ${total} ${btnWord(total)}:\n\n<code>${preview}</code>`);
        }
        catch (e) {
