...
async function handleStatsCommand(userId, chatId, arg) {
    ...
    if (!premium) {
        await (0, tg_1.sendMessage)(chatId, `📊 <b>Аналитика кликов</b> — функция Premium.\n\n` +
    ...
    if (trimmed === 'on' || trimmed === 'вкл') {
        ...
        await (0, tg_1.sendMessage)(chatId, `📊 <b>Отслеживание кликов включено!</b>\n\n` +
    ...
    if (trimmed === 'off' || trimmed === 'выкл') {
        ...
        await (0, tg_1.sendMessage)(chatId, '📊 Отслеживание кликов выключено.');
        return;
    }
    // Show stats
...
async function handlePostStats(userId, chatId, text) {
    const premium = await (0, db_1.isPremium)(userId);
    if (!premium) {
        await (0, tg_1.sendMessage)(chatId, '📊 Аналитика доступна в Premium. /premium');
        return;
    }
    const parsed = (0, parser_1.parsePostLink)(text);
    if (!parsed) {
        await (0, tg_1.sendMessage)(chatId, '❌ Не распознал ссылку на пост.');
        return;
    }
    const links = await (0, db_1.getPostStats)(userId, String(parsed.chatId), parsed.messageId);
    if (links.length === 0) {
        await (0, tg_1.sendMessage)(chatId, '📊 По этому посту нет данных. Убедись, что кнопки добавлены с отслеживанием.');
        return;
    }
    ...
    const lines = links
        ...
        .join('\n\n');
    await (0, tg_1.sendMessage)(chatId, `📊 <b>Статистика поста</b>\n\nВсего кликов: <b>${total}</b>\n\n${lines}`);
}
