...
async function handleScheduleCommand(userId, chatId, states) {
    ...
    if (!premium) {
        await (0, tg_1.sendMessage)(chatId, `⏰ <b>Отложенные кнопки</b> — функция Premium.\n\n` +
    ...
    await (0, tg_1.sendMessage)(chatId, `⏰ <b>Отложенные кнопки</b>\n\n` +
...
async function handleQueueCommand(userId, chatId) {
    ...
    if (tasks.length === 0) {
        await (0, tg_1.sendMessage)(chatId, 'У тебя нет запланированных задач.');
        return;
    }
    ...
    await (0, tg_1.sendMessage)(chatId, `⏰ <b>Запланированные задачи</b>:\n\n${lines.join('\n\n')}\n\n` +
        `Отменить: <code>/cancel_task ID</code>`);
}
async function handleCancelTask(userId, chatId, arg) {
    const id = parseInt(arg.trim(), 10);
    if (isNaN(id)) {
        await (0, tg_1.sendMessage)(chatId, '❌ Укажи ID задачи: <code>/cancel_task 42</code>');
        return;
    }
    const cancelled = await (0, db_1.cancelScheduledTask)(id, userId);
    if (cancelled) {
        await (0, tg_1.sendMessage)(chatId, `✅ Задача #${id} отменена.`);
    }
    else {
        await (0, tg_1.sendMessage)(chatId, `❌ Задача #${id} не найдена или уже выполнена.`);
    }
}
// ── State handlers ────────────────────────────────────────────────────────────
...
async function handleScheduleButtons(userId, chatId, text, state, states) {
    ...
    if (!rows) {
        await (0, tg_1.sendMessage)(chatId, `❌ Не смог разобрать кнопки. Формат: <code>Текст | URL</code>\n\n❌ /cancel — отмена`);
        return;
    }
    states.set(userId, {
    ...
    await (0, tg_1.sendMessage)(chatId, `✅ Кнопки:\n<code>${preview}</code>\n\n` +
...
async function handleScheduleTime(userId, chatId, text, state, states) {
    ...
    if (isNaN(hours) || hours < 0.1 || hours > 168) {
        await (0, tg_1.sendMessage)(chatId, '❌ Введи число от 0.1 до 168 (часы).\n\nНапример: <code>1</code>, <code>2.5</code>, <code>24</code>');
        return;
    }
