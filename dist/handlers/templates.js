...
async function handleTemplatesCommand(userId, chatId, states) {
    ...
    if (templates.length === 0) {
        const hint = premium
            ...
            : `У тебя пока нет шаблонов.\n\n✨ <b>Premium</b> — до ${db_1.PREMIUM_MAX_TEMPLATES} шаблонов. <b>Free</b> — до ${db_1.FREE_MAX_TEMPLATES}.\n\nСохрани набор кнопок: /add → добавь кнопки → потом <code>/save Название</code>`;
    ...
    await (0, tg_1.sendMessage)(chatId, `📋 <b>Твои шаблоны</b> (${templates.length}/${maxLabel}):\n\n${lines}\n\n` +
...
async function handleSaveCommand(userId, chatId, arg, states) {
    ...
    if (count >= maxT) {
        await (0, tg_1.sendMessage)(chatId, `⚠️ Лимит шаблонов: ${maxT}.\n${premium ? 'Удали ненужный шаблон командой /del.' : '✨ /premium — до ' + db_1.PREMIUM_MAX_TEMPLATES + ' шаблонов'}`);
        return;
    }
    const name = arg.trim();
    if (!name) {
        states.set(userId, { step: 'waiting_template_name_save' });
        await (0, tg_1.sendMessage)(chatId, '💾 Как назвать шаблон? Введи название:\n\n❌ /cancel — отмена');
        return;
    }
    states.set(userId, { step: 'waiting_template_buttons_save', name });
    await (0, tg_1.sendMessage)(chatId, `💾 Отправь кнопки для шаблона <b>"${name}"</b>:\n\nФормат: <code>Текст | URL</code> (каждая строка — ряд)\n\n❌ /cancel — отмена`);
}
async function handleTemplateName(userId, chatId, name, states) {
    ...
    if (count >= maxT) {
        ...
        await (0, tg_1.sendMessage)(chatId, `⚠️ Лимит шаблонов: ${maxT}. Удали ненужный командой /del.`);
        return;
    }
    states.set(userId, { step: 'waiting_template_buttons_save', name });
...
async function handleTemplateButtonsSave(userId, chatId, text, name, states) {
    ...
    if (!rows) {
        await (0, tg_1.sendMessage)(chatId, `❌ Не смог разобрать кнопки. Проверь формат (макс. ${maxButtons} кнопок).\n\n❌ /cancel — отмена`);
        return;
    }
    await (0, db_1.saveTemplate)(userId, name, text);
    states.set(userId, { step: 'idle' });
    const preview = (0, parser_1.formatButtonPreview)(rows);
    await (0, tg_1.sendMessage)(chatId, `✅ Шаблон <b>"${name}"</b> сохранён!\n\n<code>${preview}</code>\n\nПрименить: <code>/apply ${name}</code>`);
}
// ── /apply ───────────────────────────────────────────────────────────────────
async function handleApplyCommand(userId, chatId, arg, states) {
    if (!arg.trim()) {
        const templates = await (0, db_1.getTemplates)(userId);
        if (templates.length === 0) {
            await (0, tg_1.sendMessage)(chatId, 'У тебя нет шаблонов. Создай шаблон командой /save.');
            return;
        }
        const list = templates.map(t => `• <code>/apply ${t.name}</code>`).join('\n');
        await (0, tg_1.sendMessage)(chatId, `📋 Твои шаблоны:\n\n${list}`);
        return;
    }
    const name = arg.trim();
    const template = await (0, db_1.getTemplate)(userId, name);
    if (!template) {
        await (0, tg_1.sendMessage)(chatId, `❌ Шаблон <b>"${name}"</b> не найден.\n\nПосмотри список: /templates`);
        return;
    }
    states.set(userId, { step: 'waiting_template_apply_link', templateName: template.name });
    await (0, tg_1.sendMessage)(chatId, `📋 Шаблон <b>"${template.name}"</b> выбран.\n\n🔗 Теперь отправь ссылку на пост:\n\n❌ /cancel — отмена`);
}
async function handleTemplateApplyLink(userId, chatId, text, templateName, states) {
    ...
    if (!template) {
        states.set(userId, { step: 'idle' });
        await (0, tg_1.sendMessage)(chatId, '❌ Шаблон не найден. Возможно, был удалён.');
        return;
    }
    const premium = await (0, db_1.isPremium)(userId);
    if (!premium) {
        ...
        if (used >= db_1.FREE_DAILY_LIMIT) {
            states.set(userId, { step: 'idle' });
            await (0, tg_1.sendMessage)(chatId, `⚠️ Дневной лимит (${db_1.FREE_DAILY_LIMIT}) исчерпан.\n✨ /premium — безлимит`);
            return;
        }
    }
    const maxButtons = premium ? db_2.PREMIUM_MAX_BUTTONS : db_2.FREE_MAX_BUTTONS;
    ...
    if (!rows) {
        states.set(userId, { step: 'idle' });
        await (0, tg_1.sendMessage)(chatId, '❌ Шаблон повреждён. Пересохрани его командой /save.');
        return;
    }
    states.set(userId, { step: 'idle' });
    ...
    try {
        ...
        await (0, tg_1.sendMessage)(chatId, `✅ Шаблон <b>"${template.name}"</b> применён! (${total} ${btnWord(total)})`);
    }
    catch (e) {
...
async function handleDeleteTemplate(userId, chatId, arg, states) {
    if (!arg.trim()) {
        ...
        await (0, tg_1.sendMessage)(chatId, '🗑 Введи название шаблона для удаления:\n\n❌ /cancel — отмена');
        return;
    }
...
async function doDeleteTemplate(userId, chatId, name, states) {
    ...
    if (deleted) {
        await (0, tg_1.sendMessage)(chatId, `🗑 Шаблон <b>"${name}"</b> удалён.`);
    }
    else {
